...
private:
    ...
    // Pequeña ayuda para lectura exacta por titulo (búsqueda exacta)
    static bool mismosTitulos(const std::string& a, const std::string& b) {
        return a == b; // si quieres, aquí puedes hacer la comparación case-insensitive
    }
...
int main() {
    ...
    while (true) {
        ...
        if (!(std::cin >> opcion)) {
            // manejo por si el usuario mete algo no numerico
            std::cin.clear();
            limpiarBuffer();
